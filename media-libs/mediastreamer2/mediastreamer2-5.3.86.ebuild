...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="alsa av1 bv16 debug doc g726 g729 gsm jpeg matroska opengl opus pcap portaudio +pulseaudio qrcode speex srtp resample theora tools +v4l vpx yuv zrtp"
RESTRICT="test"
...
RDEPEND="net-libs/bctoolbox
	...
	pulseaudio? ( media-video/pipewire[sound-server] )
...
PATCHES=(
	"${FILESDIR}"/"${PN}"-5.3.4-pkgconfig.patch
	"${FILESDIR}"/"${PN}"-5.3.86-fix-crypto-compile.patch
)
